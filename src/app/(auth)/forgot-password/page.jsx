...
export default function ForgotPasswordPage() {
  ...
  async function submit(event) {
    ...
    setLoading(true); setError(""); setMessage(""); setDevLink("");
    try {
      ...
      if (data.developmentResetUrl) setDevLink(data.developmentResetUrl);
    } catch (err) {
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-6">
      <div className="w-full max-w-md rounded-2xl border bg-card p-8 shadow-sm">
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          ...
          {devLink && <Link className="text-sm text-primary underline break-all" href={devLink}>Open development reset link</Link>}
